...
define :db_postgres_set_privileges, :preset => "administrator", :username => nil, :password => nil, :db_name => nil do 
  ...
  ruby_block "set admin credentials" do
    block do
      ...
      username1 = conn.escape_string(username)
      password1 = conn.escape_string(password)
      # Username and password to pass for creation of user
      username = conn.quote_ident(username1)
      password = conn.quote_ident(password1)
      ...
      when 'administrator'
        ...
        result = conn.exec("SELECT COUNT(*) FROM pg_user WHERE usename='#{username1}'")
        userstat = result.getvalue(0,0)
        if ( userstat == '1' )
          Chef::Log.info "  User #{username1} already exists, updating user using current inputs"
          conn.exec("ALTER USER #{username} SUPERUSER CREATEDB CREATEROLE INHERIT LOGIN ENCRYPTED PASSWORD '#{password}'")
        else
          Chef::Log.info "  Creating administrator user #{username1}"
      ...
      when 'user'
        ...
        if ( userstat == '1' )
          ...
          Chef::Log.info "  Creating application user #{username1}"
